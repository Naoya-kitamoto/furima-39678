  ...
  describe "商品出品機能" do
    ...
    context "商品を出品できない時" do
      ...
      it "delivery_charge_idが空では出品できない" do
        ...
        @item.valid?
        expect(@item.errors.full_messages).to include("Delivery Charge Id can't be blank")
      end
      ...
      it "priceに半角数字以外が入力された場合出品できない" do
        @item.price = "こんにちは"
        @item.valid?
        expect(@item.errors.full_messages).to include("Price only use half width number")
      end
    end
  end
end
