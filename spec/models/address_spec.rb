  ...
  describe '住所情報の保存' do
    ...
    context '商品を出品できない時' do
      ...
      it 'sender_idが空だと保存できない' do
        @address.sender_id = ''
        @address.valid?
        expect(@address.errors.full_messages).to include("Sender can't be blank")
      end
      ...
      it 'post_codeが3-4の7桁の数字ではない場合保存できない' do
        ...
        @address.valid?
        expect(@address.errors.full_messages).to include('Post Cpde is invalid. Include hyphen(-)')
      end
      it 'sender_idが1の場合保存できない' do
        @address.municipalities = '1'
        @address.valid?
        expect(@address.errors.full_messages).to include("Sender Id can't be blank")
      end
      it 'phone_numberが10桁未満の場合保存できない' do
        @address.municipalities = '123456789'
        @address.valid?
        expect(@address.errors.full_messages).to include('Phone Number is invalid. Must be 10 to 11 digits')
      end
